...
module Jekyll
  ...
  class Site
    ...
    def write_archive_index(archive_dir, posts, year, month=nil)
      index = ArchiveIndex.new(self, self.source, archive_dir, year, month)
      index.data['posts'] = posts
    ...
    def write_archive_indexes
      if self.layouts.key? 'group_index'
        ...
        posts_by_year_month = self.posts.inject({}) do |h, post|
          ...
          self.write_archive_index(File.join(dir, year), posts.reverse, year)
        end
         
      # Throw an exception if the layout couldn't be found.
      else
        throw "No 'group_index' layout found."
      end
    end
  ...
  module Filters
    ...
    def archive_links(archives)
      archives.keys.sort.reverse.collect do |path|
        ...
        %Q{<link rel="archives" title="#{Months[month]} #{year}" href="/#{path}/" />}
      end
    end
    ...
    def archive_list(archives)
      archives.sort.reverse.collect do |path, count|
        year, month = path.split('/').map(&:to_i)
        %Q{<li><a href="/#{path}/" class="roll"><span data-title="#{Months[month]} #{year}">#{Months[month]} #{year}</span></a>&nbsp;(#{count})</li>}
      end
    end
